...
{
    if (!store.config.settings.keepLog)
        return;
    ...
    if (auto localStore = dynamic_cast<LocalStore *>(&store))
        ...
        logDir = store.config.settings.nixLogDir;
    Path dir = fmt("%s/%s/%s/", logDir, LocalFSStore::drvsLogDir, baseName.substr(0, 2));
    createDirs(dir);

    Path logFileName = fmt("%s/%s%s", dir, baseName.substr(2), store.config.settings.compressLog ? ".bz2" : "");
    ...
    if (store.config.settings.compressLog)
